...
namespace Box2D
{
    public class PolygonShape : Shape
    {
        ...
        {
            Debug.Assert(2 <= vertices.Length);
            _vertexCount = vertices.Length;
            _normals = new Vector2[_vertexCount];
            // Not Copy vertices.
            _vertices = vertices;
            ...
            _centroid = ComputeCentroid(_vertices);
        }

        static Vector2 ComputeCentroid(Vector2[] vs)
        {
            Debug.Assert(vs.Length >= 2);
            ...
            for (int i = 0; i < vs.Length; ++i)
	        {
		        ...
		        Vector2 p1 = pRef;
		        Vector2 p2 = vs[i];
                Vector2 p3 = i + 1 < vs.Length ? vs[i + 1] : vs[0];
